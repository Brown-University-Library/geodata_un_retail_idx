...
# **************** BACKGROUND ****************
#
# This script is designed to read data from the Retail Price Index with Details
# (RPID) spreadsheets published by the United Nations International Civil
# Service Commission (UN ICSC), and write those data into a more usable form.
# More information on this dataset and a detailed description of the data
# products generated by this script can be found on the GIS and Data Services
# Wiki (only accessible to members of the GeoData team at Brown):
# http://gisdatalib.digitalscholarship.brown.edu/doku.php?id=data:un_icsc
#
# I decided to use the xlwings library to read in spreadsheet data due to its
# specific functionalities compared to other Python libraries for Excel.  Lists
# are the primary data structure used within the script to store and append data
# iteratively, but some data are temporarily converted to pandas dataframes for
# specific operations. ISO and index code lookups are handled with dictionaries.
#
# To walk through the script code in the order of execution, scroll down to the
# READY TO RUMBLE section and start reading the inline comments from there.
#
# ******* NOTES ON PROJECT FILE STRUCTURE *********
#
# Every time the script processes a spreadsheet, that spreadsheet's file name
# is appended to a local csv file listing the names of "already done"
# spreadsheets. By keeping track of which spreadsheets have already been
# processed in this way, we can download future UN ICSC spreadsheets into a
# designated folder and then just run the script, and it will produce updated
# data based on the new spreadsheets without re-processing all of the old ones.
# Having this file also means that specific old spreadsheets can be
# re-processed by deleting those file names from the "already done" file,
# saving it, and then running the script again.
#
# This script is therefore not a "standalone" piece of code.  In addition to
# reading the aforementioned "already done files" csv, the script also reads
# and maintains local files containing supplementary information like ISO
# codes and index categories, since this was preferable to having this
# information hard-coded into the script.  When supplementary information is
# not sufficient for a particular case, the script will ask for the user to
# type in missing information while the script is running, and those
# supplemental files will be updated accordingly.
#
# In addition to relying on files within its directory or sub-folders, the
# script also reads in spreadsheets from a separate file location within the
# GIS and Data Services shared drive, which the script accesses by navigating
# up to its grandparent directory and then down a different branch of the folder
# tree.  This was an intentional choice informed by a specific set of file
# management needs for Brown.  Because of this, the script cannot be readily
# "packaged up" in a zipped folder and run elsewhere without first editing the
# file path for input spreadsheets (i.e. the variable named SHEETS_PATH) in
# this script to conform to a different directory structure.
